...
import { 
  Mail, Phone, MapPin, Send, Copy, 
  Check, Sparkles, Building, Home, MessageSquare 
} from "lucide-react";
...
const Contact = () => {
  ...
  const contactInfo = {
    ...
    presentAddress: "Flat-B2, H/N:20, R/N:06, Kaderabad Housing, Mohammadpur, Dhaka-1207, Bangladesh",
    permanentAddress: "Holding no:129, Uttorcourt gaon, Munshiganj Sodor, Munshiganj-1500, Bangladesh",
  };
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
          ...
          <p className="text-sm sm:text-base text-gray-100 max-w-2xl font-sans font-light leading-relaxed">
            Feel free to reach out for academic collaborations, research inquiries, or professional networking.
          </p>
        </div>
      </section>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Send an email for research or formal queries</p>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 text-xs sm:text-sm">
                  <div>
                    <span className="text-xs font-semibold text-emerald-800 block">Personal Mail</span>
                    <a href={`mailto:${contactInfo.personalEmail}`} className="text-gray-800 font-medium hover:text-emerald-700">
                      {contactInfo.personalEmail}
                    </a>
                  </div>
                  <button
                    onClick={() => handleCopy(contactInfo.personalEmail, "personalEmail")}
                    className="p-2 text-gray-500 hover:text-emerald-800 transition-colors"
                    title="Copy Email"
                  >
                    {copiedText === "personalEmail" ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 text-xs sm:text-sm">
                  <div>
                    <span className="text-xs font-semibold text-emerald-800 block">Institutional Mail (BUET)</span>
                    <a href={`mailto:${contactInfo.institutionalEmail}`} className="text-gray-800 font-medium hover:text-emerald-700">
                      {contactInfo.institutionalEmail}
                    </a>
                  </div>
                  <button
                    onClick={() => handleCopy(contactInfo.institutionalEmail, "institutionalEmail")}
                    className="p-2 text-gray-500 hover:text-emerald-800 transition-colors"
                    title="Copy Email"
                  >
                    {copiedText === "institutionalEmail" ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Available during standard business hours</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {contactInfo.phoneNumbers.map((phone, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 text-xs sm:text-sm">
                    <div>
                      <span className="text-xs font-semibold text-emerald-800 block">Number {idx + 1}</span>
                      <a href={`tel:${phone}`} className="text-gray-800 font-medium hover:text-emerald-700">
                        {phone}
                      </a>
                    </div>
                    <button
                      ...
                      title="Copy Number"
                    >
                      {copiedText === `phone${idx}` ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
                    </button>
                  </div>
                ))}
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Present and permanent locations</p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8 border border-emerald-100 shadow-sm space-y-6">
              ...
              {isSubmitted ? (
                <div className="p-6 rounded-xl bg-emerald-50 border border-emerald-200 text-center space-y-2">
                  ...
                  <p className="text-xs text-gray-600">Thank you for getting in touch. I will respond as soon as possible.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 px-6 rounded-xl bg-[#163A2D] text-amber-300 font-bold text-xs sm:text-sm hover:bg-[#0C2219] transition-colors flex items-center justify-center gap-2 shadow-sm"
                  >
