...
const Tools = () => {
  ...
  return (
    <div className="w-full bg-white text-slate-900 rounded-xl lg:rounded-3xl overflow-hidden font-['Playfair_Display',serif]">
      {/* ==========================================
          1. HERO BANNER
      ========================================== */}
      <section
      ...
      ========================================== */}
      <section className="py-16 px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto flex flex-col items-center bg-white rounded-b-xl lg:rounded-b-3xl">
        {/* Header Tag & Title */}
        ...
        <div className="w-full max-w-3xl flex flex-col items-center gap-6 mb-14 font-sans">
          ...
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-full py-2.5 pl-11 pr-10 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:bg-white transition-all shadow-sm focus:shadow-md"
            />
            {searchQuery && (
              <button
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 cursor-pointer ${
                    isActive
                      ...
                      : "bg-slate-100/80 border border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-200/60"
                  }`}
                >
                  {tab.label}
                </button>
        ...
        {/* Error State */}
        {error && !loading && (
          <div className="bg-red-50 p-6 rounded-2xl border border-red-200 max-w-md text-center space-y-2 font-sans my-8 shadow-sm">
        ...
        {!loading && !error && filteredTools.length === 0 && (
          <div className="text-center py-16 bg-slate-50/80 rounded-3xl border border-dashed border-slate-300 w-full max-w-md my-8 space-y-2 font-sans shadow-inner">
        ...
        {/* Dynamic Circular Tech Icons Grid */}
        {!loading && !error && filteredTools.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 sm:gap-10 w-full place-items-center mb-20">
            {filteredTools.map((tool) => (
              ...
              >
                ...
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-slate-50 border border-slate-200/80 flex items-center justify-center p-4 mb-3 shadow-sm group-hover:border-amber-400 group-hover:shadow-lg group-hover:shadow-amber-500/10 group-hover:bg-white transition-all duration-300 overflow-hidden">
                  {/* Base64 / URL Image Preview */}
                  {tool.imageUrl ? (
                    <img
                      src={tool.imageUrl}
                      alt={tool.name}
                    ...
                    <Terminal className="w-8 h-8 sm:w-10 sm:h-10 text-slate-600 group-hover:text-amber-500 transition-colors" />
                  )}
                </div>

                {/* Tool Name & Description */}
                <h3 className="text-sm font-bold text-slate-800 group-hover:text-amber-600 font-sans transition-colors line-clamp-1">
                  {tool.name}
                </h3>
                <span className="text-[11px] text-slate-500 font-sans font-normal mt-0.5 line-clamp-1">
                  {tool.desc}
                </span>
              </div>
            ))}
        ...
        {/* Footer Tagline */}
        <div className="w-full border-t border-slate-200 pt-8 text-center">
